<template>
    <div class="type-notification-component">
        <h2 class="title my-3">QUẢN LÝ TÀI KHOẢN</h2>
        <div class="bg-white">
        ...
        <v-data-table v-model:items-per-page="itemsPerPage" :headers="headers" :items="dataSource" item-value="name"
            ...
            <template v-slot:item.status="{ item }">
                <div v-if="item.raw.status == 1" class="text-success">Còn hiệu lực</div>
                <div v-else class="text-danger">Đã khoá</div>
            </template>
            <template v-slot:item.actions="{ item }">
                <button class="btn btn-primary mr-2 px-3 p-0 rounded-circle" @click="editItem(item)">
                    <i class="fas fa-info"></i>
                </button>
                <button v-if="item.raw.status == 1" class="btn btn-danger py-0" @click="deleteItem(item)">
                    <i class="fa-solid fa-lock"></i>
                </button>
                <button v-else class="btn btn-success py-0" @click="deleteItem(item)">
