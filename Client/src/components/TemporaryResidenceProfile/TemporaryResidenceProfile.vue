<template>
    <div class="temporary-residence-profile-component">
        ...
        <h4 class="title-2">Danh sách hồ sơ tạm trú</h4>
        ...
        <v-data-table v-model:items-per-page="itemsPerPage" :headers="headers" :items="dataSource" item-value="name"
            ...
            <template v-slot:item.isExpired="{ item }">
                <div v-if="item.raw.temporaryResidenceExpiration != null">
                    <div v-if="item.raw.isExpired == 1" class="text-danger">
                        Hết hạn
                    </div>
                    <div v-else class="text-success">
                        {{ "Còn " + calculateDate(item.raw.createdAt, item.raw.temporaryResidenceExpiration) + " ngày" }}
                    </div>
                </div>
...
export default defineComponent({
    ...
    setup(props, context) {
        ...
        const addTemporaryResidenceProfile = () => {
            context.emit("add:temporaryResidenceProfile", { isDelete: 0, isEpired: 0 });
            dialog.value = false;
        }
        const goAdd = () => {
