<template>
    <div class="general-profile-component mb-5">
        <div class="row m-0 my-3 justify-content-between align-center">
            <h2 class="title">HỒ SƠ GIA HẠN TẠM TRÚ</h2>
            ...
            <v-window-item value="tab-1">
                <Form @submit="submitGeneralForm" :validation-schema="generalFormSchema">
                    ...
                    <div class="form-row m-0">
                        ...
                        <div class="form-group col">
                            <label>Thời gian gia hạn đến ngày <span class="text-danger font-weight-bold">(*)</span></label>
                            <Field :disabled="statusForm" class="form-control" name="temporaryResidenceExpiration"
                                v-model="generalData.temporaryResidenceExpiration" type="date">
                            </Field>
                            <ErrorMessage name="caseProfileG" class="text-danger"></ErrorMessage>
                        </div>
                        <div class="form-group col-12">
...
export default defineComponent({
    ...
    setup(props, context) {
        ...
        const generalData = ref({
            ...
            "status": props.generalProfile?.status ? props.generalProfile.status : "",
        });
        const diffData = ref({
            typeProfile: [],
            typeNotification: [],
        ...
        const generalFormSchema = yup.object().shape({
            ...
            temporaryResidenceExpiration: yup
                .string()
                .required("Thời gian gia hạn không thể trống"),
        });
        const submitGeneralForm = async () => {
            try {
                ...
                await generalProfileService.update(props.generalProfile.id, {
                    ...
                    temporaryResidenceExpiration: generalData.value.temporaryResidenceExpiration,
                });
        ...
        const accepted = async () => {
            try {
                ...
                if (confirm("Bạn có chắc tiếp nhận hồ sơ này")) {
                    await generalProfileService.acceptedExtension(props.generalProfile.id);
                    context.emit("rerender:generalProfile");
        ...
        const denied = async () => {
            try {
                progressCircular.setState(true);
                await generalProfileService.deniedExtension(props.generalProfile.id, dataDenied.value);
                context.emit("rerender:generalProfile");
                dialog.value = false;
