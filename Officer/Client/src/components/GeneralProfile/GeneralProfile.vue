<template>
    <div class="general-profile-component">
        <h2 class="title my-3">HỒ SƠ ĐĂNG KÝ TẠM TRÚ</h2>
        <div class="bg-white">
        ...
        <div class="d-flex align-center justify-content-between mb-2">
            <h4 class="title-2">Danh sách hồ sơ đăng ký tạm trú</h4>
        ...
        <v-data-table v-model:items-per-page="itemsPerPage" :headers="headers" :items="dataSource" item-value="name"
            ...
            <template v-slot:item.status="{ item }">
                ...
                <div v-else-if="item.raw.status == 3" class="text-danger">
                    <i>Đã từ chối</i>
                </div>
                <div v-else-if="item.raw.status == 2" class="text-warning">
                    Yêu cầu bổ sung
                </div>
