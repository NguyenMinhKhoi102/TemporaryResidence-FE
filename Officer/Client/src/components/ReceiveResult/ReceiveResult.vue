<template>
    <div class="receive-result-component">
        <h2 class="title my-3">HÌNH THỨC NHẬN KẾT QUẢ</h2>
        <div class="bg-white">
                ...
                <v-card class="p-4">
                    ...
                    <Form :validation-schema="formSchema" @submit="submitForm">
                        ...
                        <div class="form-group">
                            <label for="name">Tên hình thức nhận kết quả</label>
                            <Field type="text" class="form-control" id="name" name="name" v-model="local.name"
                                placeholder="Tên hình thức nhận kết quả" />
        ...
        <v-data-table v-model:items-per-page="itemsPerPage" :headers="headers" :items="dataSource" item-value="name"
            ...
            <template v-slot:item.isActive="{ item }">
                <v-switch :loading="false" color="success" inset :model-value="item.raw.isActive == 1 ? true : false"
                    label="Bật" hide-details @click="changeStatus(item)"></v-switch>
            </template>
...
export default defineComponent({
    ...
    setup(props, context) {
        ...
        const id = ref(-1);
        const resetData = () => {
            titleDialog.value = "THÊM HÌNH THỨC NHẬN KẾT QUẢ";
            id.value = -1;
            local.value.name = "";
        }
        const editItem = (item) => {
            titleDialog.value = "SỬA HÌNH THỨC NHẬN KẾT QUẢ";
        ...
        const deleteItem = (item) => {
            if (confirm("Bạn có chắc muốn xoá")) {
                console.log(item);
                context.emit("delete:receiveResult", item.raw.id);
            }
        }
        const submitForm = () => {
            if (id.value == -1) {
                context.emit('add:receiveResult', local.value);
                dialog.value = false;
            }
            else {
                context.emit("update:receiveResult", id.value, local.value);
                dialog.value = false;
            }
        }
        const changeStatus = (item) => {
            context.emit("update:receiveResult", item.raw.id, { "name": item.raw.name, "isActive": item.raw.isActive == 1 ? 0 : 1 })
        }
        return {
