<template>
    <div class="temporary-residence-profile-component">
            ...
            <v-dialog v-model="dialog" width="auto" persistent>
                ...
                <v-card class="p-3">
                    ...
                    <v-card-actions class="justify-content-end">
                        ...
                        <button class="btn btn-outline-primary" @click="addTemporaryResidenceProfile">Đồng ý</button>
                    </v-card-actions>
                </v-card>
            </v-dialog>
        ...
        <v-data-table v-model:items-per-page="itemsPerPage" :headers="headers" :items="dataSource" item-value="name"
            ...
            <template v-slot:item.isExpired="{ item }">
                <div
                    v-html="item.raw.temporaryResidenceExpiration != null ? calculateDaysUntilDate(item.raw.temporaryResidenceExpiration) : ''">
                </div>
...
import { VCard, VIcon, VBreadcrumbs, VDialog, VCardTitle, VCardActions } from 'vuetify/lib/components/index.mjs';
...
export default defineComponent({
    ...
    emits: [
        "delete:temporaryResidenceProfile",
    ],
    setup(props, context) {
        ...
        const headers = [
            ...
            { title: 'Ngày hết hạn', key: 'temporaryResidenceExpiration' },
        ...
        const deleteItem = (item) => {
            if (confirm("Bạn có chắc muốn xoá")) {
                context.emit("delete:temporaryResidenceProfile", item.raw.id);
            }
        }
        ...
        const addTemporaryResidenceProfile = () => {
            context.emit("add:temporaryResidenceProfile", { isDelete: 0, isEpired: 0, wardId: ward.code });
            dialog.value = false;
        }
        const goAdd = () => {
        ...
        return {
            ...
            calculateDate,
            goAdd,
            addTemporaryResidenceProfile,
            dialog, setDialog, calculateDaysUntilDate
        }
    }
});
</script>
<style>
.temporary-residence-profile-component {
    .table-search {
